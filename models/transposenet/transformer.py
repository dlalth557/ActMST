...
class Transformer(nn.Module):
    ...
    def forward(self, src, query_embed, pos_embed):
        ...
        bs, c, h, w = src.shape
        src = src.flatten(2).permute(2, 0, 1) # (B, C, H, W) -> (B, C, HW) -> (HW, B, C)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1) # (C, H, W) -> (B, C, H, W) -> (B, C, HW) -> (HW, B, C)
        query_embed = query_embed.unsqueeze(1).repeat(1, bs, 1)

        tgt = torch.zeros_like(query_embed)
        memory, res_mean = self.encoder(src, pos=pos_embed)
        hs = self.decoder(tgt, memory, pos=pos_embed, query_pos=query_embed)
        return hs.transpose(1, 2), res_mean
